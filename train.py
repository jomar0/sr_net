...
def train(model, dataloaders, epochs, learning_rate, criterion, log_path = None, device='cuda'):
    ...
    for epoch in range(epochs):
        epoch_loss = 0.0
        model.train()
        progress_bar = tqdm(desc=f'Epoch {epoch+1}/{epochs} - Training  ', total=len(training_dataloader.dataset), postfix=f'Loss: 0.000')
        ...
        progress_bar.set_description_str(f"Epoch {epoch+1}/{epochs} - Evaluating")
        ...
        with torch.no_grad():
            ...
            save_state.update(epoch=epoch+1, model=model, psnr=eval_psnr, ssim=eval_ssim)
        progress_bar.close()
...
def test(model, dataloader, criterion=None, log_path=None, device='cuda'):
    if criterion == None: criterion = SSIMLoss()
    model.to(device)
    model.eval()
    loss = 0.0
    eval_psnr = 0.0
